...
from ai_strategies.strategie_No1_dev_ai import (
    StrategieNo1,
)  # Importer la stratégie spécifique
    ...
    if msg_type == "UNIT_UPDATE":
        try:
            if "id" not in data or "x" not in data or "y" not in data:
                Print_Display(
                    f"[WARNING] UNIT_UPDATE message incomplet. Clés reçues: {list(data.keys())} | Payload: {payload}"
                )
                return
...
def load_existing_game(filename):
    ...
    if loaded_units and loaded_buildings and loaded_map and loaded_ai:
        ...
        Print_Display(
            "[ERROR] Chargement échoué. Le fichier est corrompu ou n'existe pas."
        )
...
def choose_player_side_curses(stdscr):
    ...
    options = [
        "Jouer en tant que Joueur 1 (Bleu)",
        "Jouer en tant que Joueur 2 (Rouge)",
    ]
    selected = 0
                    ...
                    case 0:  # Sauvegarder
                        ...
                        while True:
                            ...
                            if key == ord("\n"):
                                if save_name.strip() == "":
                                    stdscr.addstr(
                                        6, 0, "Erreur : le nom ne peut pas être vide."
                                    )
                                    stdscr.refresh()
                                    time.sleep(2)
                                else:
                        ...
                        try:
                            save_game_state(
                                ...
                                game_map,
                                ai,
                                os.path.join(SAVE_DIR, f"{save_name}.pkl"),
                            )
                        except Exception as e:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_DOWN:
                    ...
                    if selected_option == 0:  # Sauvegarder
                        save_name = input_text_pygame(
                            screen, font, "Nom de la sauvegarde :"
                        )
                        if save_name:
                            save_game_state(
                                units,
...
def game_loop_curses(stdscr):
    ...
    if network.client is None:
            network.client = network.load_network_client(
                NETWORK_PYTHON_PORT, NETWORK_MY_PORT
                )
    # Initialiser le network client s'il n'existe pas déjà
    ...
    while True:
        ...
        messages = network.client.consume_messages()
        ...
        view_x, view_y = handle_input(
            stdscr, view_x, view_y, max_height, max_width, game_map
        )
        display_with_curses(
            stdscr, game_map, units, player_side_state, ai, view_x, view_y
        )
        last_update_time = update_game(
            units,
            buildings,
            game_map,
            ai,
        ...
        if current_time - last_network_send_time > 0.5:
            network.send_game_state_to_c(
                network.client, units, buildings, ai, player_side_state.player_side
            )
...
def game_loop_graphics():
    ...
    while running:
        ...
        view_x, view_y = handle_input_pygame(
            view_x, view_y, max_width, max_height, game_map
        )
            ...
            case SpecialCode.ENTER:  # Touche entrée
                ...
                match selected_option:
                    ...
                    case 1:  # Continuer la dernière partie
                        loaded_units, loaded_buildings, loaded_map, loaded_ai = (
                            load_game_state(DEFAULT_SAVE)
                        )
                        if (
                            loaded_units
                            and loaded_buildings
                            and loaded_map
                            and loaded_ai
                        ):
...
def start_new_game_curses(stdscr):
        ...
        while True:
            ...
            match key:
                ...
                case SpecialCode.ENTER:  # Touche Entrée
                    if value.strip() == "":
                        value = (
                            default  # Si aucune entrée, utilise la valeur par défaut
                        )
                    break
    ...
    try:
        ...
        stdscr.addstr(
            len(input_fields) + 2,
            0,
            "Erreur : Entrée invalide, utilisation des valeurs par défaut.",
        )
        stdscr.refresh()
        time.sleep(2)
        map_size = 120
    ...
    villager1 = Unit(
        "Villager", starting_x_V1, starting_y_V1, None
    )  # Créez l'unité sans AI pour l'instant
    ...
    for unit in units:
        unit.ai = ai
        Print_Display(
            f"[INIT] Unité locale créée: {unit.unit_type} (is_remote={getattr(unit, 'is_remote', False)})"
        )
    ...
    try:
        ...
        time.sleep(0.5)
        Print_Display(
            f"[INFO] Lancement du processus réseau C : {GAMEP2P_EXE} {NET_ME} {NET_DEST} {PY_PORT}"
        )
    except Exception as e:
...
def start_new_game_graphics(screen, font):
    ...
    while running:
        ...
        for idx, (prompt, value) in enumerate(
            zip([f[0] for f in input_fields], input_values)
        ):
    ...
    try:
        ...
        network.client.send(game_map.to_network_message())
    except Exception:
