...
def display_with_curses(stdscr, game_map, units, view_x, view_y, max_height, max_width):
    ...
    for y in range(view_y, min(view_y + max_height, game_map.height)):
        for x in range(view_x, min(view_x + max_width, game_map.width)):
            ...
            if (x, y) in unit_positions:
                ...
                stdscr.addch(y - view_y, x - view_x, '.', curses.color_pair(3))  # Tuile vide en blanc
    stdscr.refresh()
